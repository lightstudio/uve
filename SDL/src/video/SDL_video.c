 ...
 */
int
SDL_VideoInit(const char *driver_name,void *driverdata)
{
    ...
    if (driver_name != NULL) {
        for (i = 0; bootstrap[i]; ++i) {
            if (SDL_strncasecmp(bootstrap[i]->name, driver_name, SDL_strlen(driver_name)) == 0) {
				video = bootstrap[i]->create(index,driverdata);
                break;
            }
        }
    } else {
	...
	if (_this->VideoInit(_this,driverdata) < 0) {
        SDL_VideoQuit();
        return -1;
    }

...
WinPhone_WindowData GlobalWData;
...
int
SDL_InitSubSystem(Uint32 flags,void *driverdata)
{
#if !SDL_TIMERS_DISABLED
    if (!ticks_started) {
        SDL_StartTicks();
    ...
    if ((flags & SDL_INIT_VIDEO) ){
        ...
        if (SDL_PrivateShouldInitSubsystem(SDL_INIT_VIDEO)) {
            if (SDL_VideoInit(NULL,driverdata) < 0) {
                return (-1);
            }
        }
...
int
SDL_Init(Uint32 flags,void* driverdata)
{
	...
	if (SDL_InitSubSystem(flags,driverdata) < 0) {
        return (-1);
    }

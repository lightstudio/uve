...
void Drft::clear()
{
    if (trigcache != nullptr)
        trigcache = nullptr;
    
    if (splitcache != nullptr)
        splitcache = nullptr;
}
...
float Drft::tpi = 6.28318530717958647692528676655900577f;

float Drft::hsqt2 = .70710678118654752440084436210485f;

float Drft::taui = .86602540378443864676372317075293618f;

float Drft::taur = -.5f;

float Drft::sqrt2 = 1.4142135623730950488016887242097f;

void Drft::drfti1(int n, float wa, int index, int ifac)
{
    ...
    for (k1 = 0; k1 < nfm1; k1++)
    {
        ...
        for (j = 0; j < ipm; j++)
        {
            ...
            for (ii = 2; ii < ido; ii += 2)
            {
                ...
                arg = fi * argld;
                wa[index + i++] = safe_cast<float>(Math::Cos(arg));
                wa[index + i++] = safe_cast<float>(Math::Sin(arg));
            }
...
void Drft::fdrffti(int n, float wsave, int ifac)
{
    if (n == 1)
        return ;
...
void Drft::dradf2(int ido, int l1, float cc, float ch, float wa1, int index)
{
    int i, k;
    float ti2, tr2;
...
void Drft::dradf4(int ido, int l1, float cc, float ch, float wa1, int index1, float wa2, int index2, float wa3, int index3)
{
...
void Drft::dradfg(int ido, int ip, int l1, int idl1, float cc, float c1, float c2, float ch, float ch2, float wa, int index)
{
    ...
    int idp2, ipp2;
    arg = tpi / safe_cast<float>(ip);
    dcp = safe_cast<float>(Math::Cos(arg));
    dsp = safe_cast<float>(Math::Sin(arg));
    ipph = (ip + 1) >> 1;
    ipp2 = ip;
    idp2 = ido;
    nbd = (ido - 1) >> 1;
...
void Drft::drftf1(int n, float c, float ch, float wa, int ifac)
{
    int i, k1, l1, l2;
    int na, kh, nf;
...
void Drft::dradb2(int ido, int l1, float cc, float ch, float wa1, int index)
{
    int i, k, t0, t1, t2, t3, t4, t5, t6;
    float ti2, tr2;
    t0 = l1 * ido;
...
void Drft::dradb3(int ido, int l1, float cc, float ch, float wa1, int index1, float wa2, int index2)
{
...
void Drft::dradb4(int ido, int l1, float cc, float ch, float wa1, int index1, float wa2, int index2, float wa3, int index3)
{
...
void Drft::dradbg(int ido, int ip, int l1, int idl1, float cc, float c1, float c2, float ch, float ch2, float wa, int index)
{
    int idij, ipph = 0, i, j, k, l, ik, iis, t0 = 0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10 = 0, t11, t12;
    float dc2, ai1, ai2, ar1, ar2, ds2;
    int nbd = 0;
    float dcp = 0, arg, dsp = 0, ar1h, ar2h;
    int ipp2 = 0;
    ...
    dcp = safe_cast<float>(Math::Cos(arg));
    dsp = safe_cast<float>(Math::Sin(arg));
    nbd = safe_cast<int>((safe_cast<unsigned int>((ido - 1)) >> 1));
    ipp2 = ip;
    ipph = safe_cast<int>((safe_cast<unsigned int>((ip + 1)) >> 1));
...
void Drft::drftb1(int n, float c, float ch, float wa, int index, int ifac)
{
    int i, k1, l1, l2 = 0;
    int na;

 ...
 *
 * $Id: player.c,v 1.68 2004/02/17 02:26:43 rob Exp $
 */
 ...
 */
static
void show_rgain(struct tag_rgain const *rgain)
{
  char const *name, *originator;

  name = 0;
  switch (rgain->name) {
  case TAG_RGAIN_NAME_NOT_SET:
    break;
  case TAG_RGAIN_NAME_RADIO:
    name = _("Radio Gain");
    break;
  case TAG_RGAIN_NAME_AUDIOPHILE:
    name = _("Audiophile Gain");
    break;
  }
  if (name == 0)
    return;
  ...
  case TAG_RGAIN_ORIGINATOR_UNSPECIFIED:
    return;
  case TAG_RGAIN_ORIGINATOR_PRESET:
    originator = _("preset");
    break;
  case TAG_RGAIN_ORIGINATOR_USER:
    originator = _("user");
    break;
  case TAG_RGAIN_ORIGINATOR_AUTOMATIC:
    originator = _("automatic");
    break;
  }

  detail(name, originator ? "%+.1f dB (%s)" : "%+.1f dB",
	 rgain->adjustment / 10.0, originator);
}
...
# endif
    ...
    if (tag->lame.replay_gain[0].name == TAG_RGAIN_NAME_RADIO)
      show_rgain(&tag->lame.replay_gain[0]);
    if (tag->lame.replay_gain[1].name == TAG_RGAIN_NAME_AUDIOPHILE)
 ...
 */
static
void use_rgain(struct player *player, struct tag_rgain *list)
{
  struct tag_rgain *rgain = &list[0];

  if ((player->output.replay_gain & RGAIN_AUDIOPHILE) &&
      list[1].name == TAG_RGAIN_NAME_AUDIOPHILE &&
      list[1].originator != TAG_RGAIN_ORIGINATOR_UNSPECIFIED)
    rgain = &list[1];

  if ((rgain->name == TAG_RGAIN_NAME_RADIO ||
       rgain->name == TAG_RGAIN_NAME_AUDIOPHILE) &&
      rgain->originator != TAG_RGAIN_ORIGINATOR_UNSPECIFIED) {
    double gain = (double) rgain->adjustment / 10;
	...
	break;
      ...
      detail(_("Replay Gain"), _("%+.1f dB %s adjustment (%s)"),
	     gain, (rgain->name == TAG_RGAIN_NAME_RADIO) ?
	     _("radio") : _("audiophile"),
	     originator ? originator : _("other"));
    }
			    ...
			    struct mad_frame *frame)
{
  ...
  if (player->stats.absolute_framecount == 0) {
	...
	if ((tag->flags & TAG_LAME) &&
	    (player->output.replay_gain & RGAIN_ENABLED) &&
	    !(player->output.replay_gain & RGAIN_SET))
...
void process_id3(struct id3_tag const *tag, struct player *player)
{
  ...
  if ((player->options & PLAYER_OPTION_SHOWTAGSONLY) ||
	...
	mad_fixed_t peak;
	struct tag_rgain rgain[2];

	mad_bit_init(&ptr, data);

	peak = mad_bit_read(&ptr, 32) << 5;

	tag_parse_rgain(&rgain[0], &ptr);
	tag_parse_rgain(&rgain[1], &ptr);
